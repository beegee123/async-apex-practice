  ...
  return (
    <button
      ...
      style={{
        ...
        border: `1px solid ${copied ? "#22c55e" : "#ffffff22"}`,
...
function AITutor({ topic, chatHistory, setChatHistory }) {
...
Common gotchas for this topic: ${topic.gotchas.join("; ")}`,
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: 340, background: "#0a0f1a", border: "1px solid #1e293b", borderRadius: 12 }}>
      <div style={{ padding: "10px 16px", borderBottom: "1px solid #1e293b", display: "flex", alignItems: "center", gap: 8 }}>
        ...
        <span style={{ color: "#64748b", fontSize: 12, fontFamily: "monospace" }}>AI Tutor — {topic.title}</span>
      </div>
        ...
        {messages.map((m, i) => (
          <div key={i} style={{ display: "flex", justifyContent: m.role === "user" ? "flex-end" : "flex-start" }}>
            <div style={{
              ...
              background: m.role === "user" ? topic.color + "22" : "#1e293b",
              border: `1px solid ${m.role === "user" ? topic.color + "44" : "#334155"}`,
              color: "#e2e8f0",
              fontSize: 13,
              lineHeight: 1.6,
              ...
              <ReactMarkdown
                ...
                components={{
                  code({node, inline, className, children, ...props}) {
                    return inline ? (
                      <code style={{background:'#1e293b', padding:'2px 6px', borderRadius:4, fontSize:12, fontFamily:'monospace'}}>
                        {children}
                      </code>
                    ) : (
                      <pre style={{background:'#0a0f1a', border:'1px solid #334155', borderRadius:8, padding:'12px 16px', overflowX:'auto'}}>
                        <code style={{fontFamily:'monospace', fontSize:12, color:'#94a3b8', lineHeight:1.6}}>
                          {children}
                        </code>
                      </pre>
                    );
                  ...
                  th({children}) {
                    return <th style={{border:'1px solid #334155', padding:'6px 12px', background:'#1e293b', textAlign:'left'}}>{children}</th>;
                  },
                  td({children}) {
                    return <td style={{border:'1px solid #334155', padding:'6px 12px'}}>{children}</td>;
                  },
                }}
              >
                {m.content}
...
export default function AsyncApexPractice() {
  ...
  return (
    <div style={{
      ...
      {/* Header */}
      <div style={{ borderBottom: "1px solid #1e293b", padding: "20px 32px", background: "#080d18" }}>
            ...
            {TOPICS.map((t) => {
              ...
              return done > 0 ? (
                <div key={t.id} style={{
                  ...
                  fontSize: 11, color: t.color, fontFamily: "monospace",
                }}>
                  {t.label} {done}/{total}
                </div>
              ) : null;
      ...
      <div style={{ display: "flex", minHeight: "calc(100vh - 73px)" }}>
        {/* Sidebar */}
        <div style={{ width: 180, borderRight: "1px solid #1e293b", padding: "20px 0", background: "#080d18", flexShrink: 0 }}>
          {TOPICS.map((t) => {
            const { done, total } = topicProgress(t.id);
            ...
            return (
              ...
              >
                ...
                <div style={{ fontSize: 13, fontWeight: active ? 600 : 400, color: active ? t.color : "#64748b", fontFamily: "monospace" }}>
                  {t.label}
                </div>
                {done > 0 && (
          ...
          <div style={{ padding: "24px 20px 0", borderTop: "1px solid #1e293b", marginTop: 16 }}>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "28px 32px", maxWidth: 900 }}>
          ...
          <div style={{ marginBottom: 24 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
              ...
              <h1 style={{ margin: 0, fontSize: 24, fontWeight: 700, color: topic.color }}>
                {topic.title}
              </h1>
            </div>
          ...
          {/* Code block */}
          <div style={{ marginBottom: 24, background: "#0a0f1a", border: "1px solid #1e293b", borderRadius: 12, overflow: "hidden" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginBottom: 24 }}>
            ...
            <div style={{ background: "#0a0f1a", border: "1px solid #1e293b", borderRadius: 12, padding: 20 }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: "#64748b", fontFamily: "monospace", marginBottom: 14, letterSpacing: 1, textTransform: "uppercase" }}>
                Exercises
              </div>
              {topic.exercises.map((ex, i) => {
                ...
                return (
                  <div
                    ...
                    style={{ display: "flex", gap: 10, padding: "8px 0", cursor: "pointer", alignItems: "flex-start", borderBottom: i < topic.exercises.length - 1 ? "1px solid #0f172a" : "none" }}
                  >
                    <div style={{
                      width: 18, height: 18, borderRadius: 4, flexShrink: 0, marginTop: 1,
                      border: `2px solid ${done ? topic.color : "#334155"}`,
                    ...
                    <span style={{ fontSize: 13, color: done ? "#475569" : "#94a3b8", textDecoration: done ? "line-through" : "none", lineHeight: 1.5 }}>
                      {ex}
                    </span>
                  </div>
                );
            ...
            <div style={{ background: "#0a0f1a", border: "1px solid #1e293b", borderRadius: 12, padding: 20 }}>
              ...
              {topic.gotchas.map((g, i) => (
                <div key={i} style={{ display: "flex", gap: 8, padding: "7px 0", borderBottom: i < topic.gotchas.length - 1 ? "1px solid #0f172a" : "none" }}>
                  <span style={{ color: "#f59e0b", fontSize: 12, marginTop: 2, flexShrink: 0 }}>▸</span>
                  <span style={{ fontSize: 12.5, color: "#94a3b8", lineHeight: 1.5 }}>{g}</span>
                </div>
              ))}

              {/* CRON helper for scheduled tab */}
              {topic.id === "scheduled" && (
                <div style={{ marginTop: 16, paddingTop: 16, borderTop: "1px solid #1e293b" }}>
                  <div style={{ fontSize: 11, color: "#475569", fontFamily: "monospace", marginBottom: 8, letterSpacing: 1, textTransform: "uppercase" }}>CRON Quick Reference</div>
                  {CRON_EXAMPLES.map((c, i) => (
                    <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "4px 0" }}>
                      <span style={{ fontSize: 11, color: "#64748b" }}>{c.label}</span>
                      <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                        <code style={{ fontSize: 11, color: "#fb923c", background: "#1e293b", padding: "2px 6px", borderRadius: 4 }}>{c.expr}</code>
                        <CopyButton text={c.expr} />
                      </div>
                    </div>
                  ))}
                </div>
          ...
          {/* AI Tutor */}
          <div>
            <div style={{ fontSize: 13, fontWeight: 700, color: "#64748b", fontFamily: "monospace", marginBottom: 12, letterSpacing: 1, textTransform: "uppercase" }}>
              💬 Ask the AI Tutor
            </div>
            <AITutor 
              topic={topic} 
        ...
        <div style={{ width: 200, borderLeft: "1px solid #1e293b", padding: 20, background: "#080d18", flexShrink: 0 }}>
          <div style={{ fontSize: 11, color: "#334155", fontFamily: "monospace", letterSpacing: 1, textTransform: "uppercase", marginBottom: 16 }}>
            Quick Compare
          </div>
          {[
            ...
            <div key={i} style={{ marginBottom: 12 }}>
              <div style={{ fontSize: 10, color: "#475569", marginBottom: 4, fontFamily: "monospace" }}>{row.label}</div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 2 }}>
                {[
                  ...
                  <div key={cell.key} style={{
                    ...
                    background: activeId === cell.key ? cell.color + "15" : "#0a0f1a",
                    borderRadius: 4, border: `1px solid ${activeId === cell.key ? cell.color + "40" : "#1e293b"}`,
                  }}>
                    {cell.val}
                  </div>
                ))}
          ...
          <div style={{ marginTop: 12, display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 2 }}>
            {TOPICS.map((t) => (
              <div key={t.id} style={{ textAlign: "center", fontSize: 9, color: t.color, fontFamily: "monospace" }}>
                {t.icon}
              </div>
            ))}
          </div>
